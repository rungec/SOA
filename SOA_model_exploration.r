...
acfplotfun <- function(currmod){
  ...
  par(mfrow=c(1,2))
  acf(resid(currmod$lme, type="normalized"), lag=20, main="ACF")
  pacf(resid(currmod$lme, type="normalized"), lag=20, main="PACF")
}
...
#Basic model, ignoring temporal autocorrelation
mod1 <- gamm(znorm ~ year, random=list(Region=~1), correlation=NULL, data=currdf)
#Add in temporal autocorrelation with 1yr lag
mod2 <- gamm(znorm ~ year, random=list(Region=~1), correlation = corAR1(form = ~ year), data=currdf)
```
