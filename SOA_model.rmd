...
### Models
We test 4 models:  
1. a gam model of z-normalised value ~ smoothed year and look to see if there is any temporal autocorrelation  
2. We then add in the autocorrelation term, and see if the model performs better.  
3. a gamm model of z-normalised value ~ smoothed year with Region as a random effect, and look for autocorrelation.  
4. the full gamm model of z-normalised value ~ smoothed year with Region as a random effect, and with an autocorrelation term with lag=1.



```{r models, include=TRUE, warning=FALSE, cache=T, dependson=c("packages and data", "merge data")}
#znormdat <- read.csv("All_industries_znorm.csv", header=TRUE, fileEncoding = "UTF-8-BOM")

#set up function to plot the model outputs
acfplotfun <- function(currmod, corrinmod){
  if(corrinmod==TRUE){
    currtitle = paste(currmod$gam$formula, "+ corr(yr)", sep=" ")
  } else {
    currtitle = currmod$gam$formula
  }
  #plot the model autocorrelation and acf plot
  acfvals <- with(pacf(resid(currmod$lme, type="normalized"), lag=20, plot=F), data.frame(lag, acf))
  p1 <- ggplot(acfvals, aes(lag, acf)) +
        geom_hline(aes(yintercept = 0)) +
        geom_segment(mapping = aes(xend = lag, yend = 0)) +
        ggtitle()
  p2 <- ggplot(currdf, aes(y=resid(currmod$lme), x=currdf$year, col=Region))+
        geom_point() +
        geom_line() +
  ...
  print(summary(currmod$gam))
  print(summary(currmod$lme))
  print(anova(currmod$gam))
  #plot the model
}
...
#Main processing
################
currdf <- znormdat %>% filter(subIndustry=="Cruise_tourism")

#Basic model, ignoring temporal autocorrelation
mod1 <- gamm(znorm ~ s(year), random=NULL, correlation=NULL, data=currdf)
acfplotfun(mod1, corrinmod=FALSE) #examine the autocorrelation

#Add in temporal autocorrelation
mod2 <- gamm(znorm ~ s(year), correlation = corCAR1(value = 0.2, form = ~ year | Region), data=currdf)
acfplotfun(mod2, corrinmod=TRUE)

#add a random effect
mod3 <- gamm(znorm ~ s(year), random=list(Region=~1), data=currdf)
acfplotfun(mod3, corrinmod=FALSE)

#add a random effect and autocorrelation
mod4 <- gamm(znorm ~ s(year), random=list(Region=~1), correlation = corCAR1(value = 0.2, form = ~ year | Region), data=currdf)
acfplotfun(mod4, corrinmod=TRUE)
...
currdf2 <- currdf %>% add_predictions(mod4)
...
```{r growthrates, include=FALSE, cache=T, dependson=c("packages and data", "merge data"), eval=FALSE}
...
# Where data is missing for several years, the annual growth rate is calculated from the two time points we have data for. 
...
```{r plot growthrates, echo=FALSE, warning=FALSE, include=FALSE, cache=T, dependson=c("packages and data", "merge data", "growthrates")}
...
```{r models, echo=FALSE, warning=FALSE, cache=T, dependson=c("packages and data", "merge data", "growthrates")}
#set up data
...
```{r models_simple, echo=FALSE, warning=FALSE, dependson=c("packages and data", "merge data", "growthrates", "models")}
...
```{r models_base, echo=FALSE, dependson=c("packages and data", "merge data", "growthrates", "models")}
