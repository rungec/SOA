...
```{r packages and data, include=FALSE, cache=F}
require(knitr)
require(tidyverse)
require(lubridate)
...
```{r, include=FALSE, cache=F, dependson=c("packages and data", "merge data")}
...
specifying the correct random effects is not so straightforward

```{r randomplots, include=TRUE, warning=TRUE, cache=F, dependson=c("packages and data", "merge data"), fig.height=6, fig.width=8}
...
```{r modelplotfuns, include=FALSE, warning=TRUE, cache=T, dependson=c("packages and data", "merge data")}
...
```{r modelsgam, include=TRUE, warning=TRUE, cache=F, dependson=c("packages and data", "merge data", "modelplotfuns", "znorm"), fig.height=6, fig.width=12}
...
#Model 1
mod1 <- gam(znorm ~ s(year) + s(year, Region, bs='fs'), data=currdf) #fs or re?? I don't think the random componenets are specified correctly
itsadugsummary(mod1)
...
```{r models_gamm, include=TRUE, warning=TRUE, cache=F, dependson=c("packages and data", "merge data", "modelplotfuns", "znorm")}
################
#Main processing
################
industries <- unique(znormdat$subIndustry)
