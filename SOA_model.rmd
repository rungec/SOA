...
comparemodelsplot <- function(m1, m2, currlabels){
    #set up predictions  
    grid <- currdf %>% 
  ...
  ggplot(currdf, aes(year, znorm, color=Region)) +
    ...
    annotate("text", 
             ...
             col=c("black", "blue"), size=4, hjust=0, label=currlabels) +
...
plotmodelbyregion <- function(m1){
  ...
  grid <- currdf %>% 
      data_grid(year, Region) %>%
      gather_predictions(m1)
  #plot 
  ggplot(currdf, aes(year, znorm)) +
    geom_point() +
    geom_line(data=grid, aes(x=year, y=pred), col="black", inherit.aes = FALSE) +
    facet_wrap(~Region) +
...
```{r models, include=FALSE, warning=FALSE, cache=T, dependson=c("packages and data", "merge data", "modelplotfuns", "znorm")}
################
#Main processing
################
currdf <- znormdat %>% filter(subIndustry=="Cruise_tourism") %>% droplevels()
print(i)
...
mod2b <- gamm(znorm ~ s(year), correlation = corARMA(form = ~ year | Region, p=2), data=currdf) #2yr lag
anova(mod2a$lme, mod2b$lme, mod2c$lme) #any difference?
anova(mod1$lme, mod2a$lme) #any difference?
acfplotfun(mod2a) #any correlation remaining?

#compare mod1 and mod2
comparemodelsplot(mod1$gam, mod2a$gam, c("Uncorrelated Errors","AR(1) Errors"))
...
mod4 <- gamm(znorm ~ s(year), random=list(Region=~1+year), correlation = corAR1(form = ~ year | Region), data=currdf)
