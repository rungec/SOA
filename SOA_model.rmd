...
dfmods <- anngrowthdf %>% 
  ...
  do(basemod = lm(anngrowth ~ I(year-2017) + relevel(Region2, ref="Norway"), data=.), 
     intmod = lm(anngrowth ~ I(year-2017) * relevel(Region2, ref="Norway"), data=.),
...
```{r models_old, echo=FALSE, warning=FALSE}
printfun <- function(data){
        print(summary(data[[2]][[1]]))
        #print(summary(data[[3]][[1]]))
